...
export class ResourceDialogComponent implements OnInit, OnDestroy {
  ...
  openDialog(id?: number) {
    this.store.dispatch(new BrowseResourceAnalysisAction.ToggleDialog({ resourceId: id }));
  }
  ngOnDestroy(): void {
    this.destroy$.next();
  ...
  async submit() {
    ...
    if (this.editMode) {
      this.store
        .dispatch(new BrowseResourceAnalysisAction.UpdateResourceAnalysis(resource));

    } else {
      this.store
        .dispatch(new BrowseResourceAnalysisAction.CreateResourceAnalysis(resource));
    }
  }
