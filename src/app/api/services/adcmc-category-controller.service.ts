...
export class AdcmcCategoryControllerService extends BaseService {
  ...
  /**
   * Path part for operation getById10
   */
  static readonly GetById10Path = '/v1/assets/main-category/{id}';

  /**
   * This method provides access to the full `HttpResponse`, allowing access to response headers.
   * To access only the response body, use `getById10()` instead.
   *
   * This method doesn't expect any request body.
   */
  getById10$Response(params: {
    id: number;
  }): Observable<StrictHttpResponse<RestApiResponseAdcmcCategory>> {

    const rb = new RequestBuilder(this.rootUrl, AdcmcCategoryControllerService.GetById10Path, 'get');
    if (params) {
   ...
   * To access the full response (for headers, for example), `getById10$Response()` instead.
   *
   * This method doesn't expect any request body.
   */
  getById10(params: {
    ...
    return this.getById10$Response(params).pipe(
