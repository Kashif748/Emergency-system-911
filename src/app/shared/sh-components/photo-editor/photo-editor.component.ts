...
export class PhotoEditorComponent implements OnInit {
  ...
  public imgUrl$ = new Subject<string>();
  ...
  @Output() imageCroppedEvent = new EventEmitter<CroppedEvent>();
  ...
  @Input() set source(data: File | string | any) {
    if (data instanceof Event) {
      ...
      if (this.FILES_REGEX.test(file.name)) {
        ...
        reader.onload = (e: any) => {
          this.imgUrl$.next(e.target.result);
        };
      } else {
      ...
      if (this.FILES_REGEX.test(data.name)) {
        ...
        reader.onload = (ev: any) => {
          this.imgUrl$.next(ev.target.result);
        };
  ...
  export() {
    ...
    const outputImage = cropedImage.toDataURL(
      'image/' + this.format,
      this.quality
    );
    cropedImage.toBlob(
